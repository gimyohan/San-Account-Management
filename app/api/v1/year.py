...
from app.models.year import YearRead, YearCreate
...
@router.get("", status_code=status.HTTP_200_OK, response_model=SuccessResponse[list[YearRead]])
...
@router.post("", status_code=status.HTTP_201_CREATED, response_model=SuccessResponse[YearRead])
def create_year(request: YearCreate, service: YearService = Depends(get_year_service), role: str = Depends(get_current_user)):
    if role != "admin":
        raise ForbiddenException()
    return SuccessResponse(data=service.create_year(request))

@router.put("/{year_id}", status_code=status.HTTP_200_OK, response_model=SuccessResponse[YearRead])
def update_year(year_id: int, request: YearCreate, service: YearService = Depends(get_year_service), role: str = Depends(get_current_user)):
    if role != "admin":
        raise ForbiddenException()
