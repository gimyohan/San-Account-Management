...
from app.models.auth import RoleRead, CodeRead, CodeUpdate
...
def login_for_access_token(response: Response, form_data: OAuth2PasswordRequestForm = Depends(), service: AuthService = Depends(get_auth_service)) -> SuccessResponse[RoleRead]:
...
async def get_current_user(role: str = Depends(auth.get_current_user)):
...
async def get_codes(limit: int = 5, service: AuthService = Depends(get_auth_service), role: str = Depends(auth.get_current_user)):
    if role != "admin":
...
async def create_code(service: AuthService = Depends(get_auth_service), role: str = Depends(auth.get_current_user)):
    if role != "admin":
        raise ForbiddenException()
...
async def update_code(
    ...
    if role != "admin":
        raise ForbiddenException()
    return SuccessResponse(data=service.update_code_memo(id, request.memo), message="메모가 저장되었습니다.")
