...
export function CategoryDetailPanel({ activeNode, activeDepth, invalidate, onSelect }: CategoryDetailPanelProps) {
  ...
  const handleCreateRoot = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && e.currentTarget.value.trim()) {
      createMutation.mutate({ name: e.currentTarget.value.trim(), parent_id: null });
      e.currentTarget.value = '';
    }
  };

  const handleCreateChild = () => {
    if (activeNode && newChildName.trim()) {
      createMutation.mutate({ name: newChildName.trim(), parent_id: activeNode.id });
    }
  };

  const handleUpdateTitle = () => {
    if (activeNode && titleInputRef.current?.value.trim()) {
      const newName = titleInputRef.current.value.trim();
      if (newName !== activeNode.name) {
        updateMutation.mutate({ id: activeNode.id, data: { name: newName, parent_id: activeNode.parent_id } });
      } else {
        setIsEditingTitle(false);
      }
    } else {
  ...
  const handleUpdateChild = (child: CategoryTree) => {
    if (childEditRef.current?.value.trim()) {
      const newName = childEditRef.current.value.trim();
      if (newName !== child.name) {
        updateMutation.mutate({ id: child.id, data: { name: newName, parent_id: child.parent_id } });
      } else {
        setEditingChildId(null);
      }
    } else {
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={(e) => {
                  ...
                  if (input.value.trim()) {
                    createMutation.mutate({ name: input.value.trim(), parent_id: null });
                    input.value = '';
                  }
                }}
  ...
  return (
    <div className="flex-1 flex flex-col bg-white/50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-white/30 dark:bg-slate-950/30">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-sm text-destructive/60 hover:bg-destructive/10 hover:text-destructive px-3 py-2 rounded-xl transition-all font-medium flex items-center gap-1.5 flex-shrink-0 ml-4"
          >
            <Trash2 size={16} />
            삭제
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="space-y-2">
          {activeNode.children?.map(child => (
            <div
              key={child.id}
              className="group flex items-center justify-between p-3.5 bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-xl hover:border-primary/30 transition-all cursor-pointer"
            >
              <div className="flex items-center gap-3 flex-1 min-w-0" onClick={() => onSelect(child.id)}>
                ...
                {editingChildId === child.id ? (
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') handleUpdateChild(child);
                      if (e.key === 'Escape') setEditingChildId(null);
                    }}
                    onBlur={() => handleUpdateChild(child)}
                  />
                ) : (
                  <span className="font-medium text-sm text-slate-700 dark:text-slate-200 group-hover:text-primary transition-colors truncate">
                    {child.name}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 ml-2">
                {child.children && child.children.length > 0 && (
                  <span className="text-[10px] text-slate-400 mr-1">{child.children.length}개</span>
                )}
                <button
      ...
      {/* 하위 생성 폼 - 항상 하단에 고정 */}
      {activeDepth < 3 && (
        <div className="p-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 px-4 py-3 rounded-xl outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary placeholder:text-slate-400 dark:placeholder:text-slate-600 transition-all text-sm"
            />
            <button
              onClick={handleCreateChild}
              disabled={!newChildName.trim() || createMutation.isPending}
              className="px-4 py-3 bg-primary text-white dark:bg-white dark:text-slate-900 rounded-xl font-bold text-sm disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition-all flex items-center gap-1.5"
            >
              <Plus size={16} />
              추가
            </button>
